      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gold/10 border border-gold/30 text-gold text-xs font-semibold uppercase tracking-[0.2em] mb-4"
          >
            <Sparkles className="h-3.5 w-3.5" /> Make Your Own Set
          </motion.div>
          <motion.h1
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Build Your Custom Skincare Ritual
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Select any 2 or more luxury items to create your personalized bundle and enjoy an instant <span className="text-gold font-medium">10% discount</span>.
          </motion.p>
        </div>

        {/* Main Interface Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          {/* Step 1: Product Selection Grid (8 cols on desktop) */}
          <div className="lg:col-span-8 space-y-6">
            <div className="flex items-center justify-between border-b border-border/40 pb-4">
              <div>
                <h2 className="font-display text-xl sm:text-2xl text-foreground">Step 1 — Choose Products</h2>
                <p className="text-xs text-muted-foreground mt-0.5">Select at least 2 items below</p>
              </div>
          ...
          {/* Step 2 & 3: Bundle Summary Tray (4 cols on desktop) */}
          <div className="lg:col-span-4 sticky top-28">
            <div className="rounded-2xl bg-secondary/30 border border-border/60 p-6 shadow-xl space-y-6">
              <div className="border-b border-border/40 pb-4">
                <p className="text-[10px] uppercase tracking-[0.3em] text-gold mb-1 font-bold">Step 2 — Review Set</p>
                <h3 className="font-display text-xl text-foreground">Custom Bundle Summary</h3>
              </div>
              ...
              <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
                {selectedProducts.length === 0 ? (
                  <div className="text-center py-8 px-4 rounded-xl border border-dashed border-border/50 text-muted-foreground text-xs">
                    No products selected yet. Click products on the left to add them to your custom bundle.
                  </div>
                ) : (
              ...
              <button
                ...
                className="w-full rounded-full bg-gold py-4 px-6 text-xs font-semibold uppercase tracking-[0.2em] text-background hover:bg-gold/90 transition-all flex items-center justify-center gap-2 shadow-lg active:scale-98 min-h-[48px]"
              >
